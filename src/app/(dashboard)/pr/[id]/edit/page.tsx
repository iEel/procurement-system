...
export default function EditPRPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchMasterData = async () => {
        try {
            ...
            const [compData, branchData, deptData] = await Promise.all([
                compRes.json(),
                branchRes.json(),
                deptRes.json(),
            ]);
    ...
    const fetchPR = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (pr.status !== "Draft") {
                    alert("ไม่สามารถแก้ไขได้ เนื่องจากใบขอซื้อไม่ได้อยู่ในสถานะ Draft");
                    router.push(`/pr/${resolvedParams.id}`);
                    return;
                }
                ...
                setPurchaseMethod(pr.purchaseMethod || "procurementHandle");
                setRequiredDate(pr.requiredDate ? pr.requiredDate.split("T")[0] : "");
                setBudget(pr.budget ? pr.budget.toString() : "");
                setRemarks(pr.remarks || "");
                setVatRate(pr.vatRate || 7);
                setWithholdingTaxRate(pr.withholdingTaxRate || 0);
                setDiscountAmount(pr.discountAmount || 0);

                // Set items
                if (pr.items && pr.items.length > 0) {
                    setItems(pr.items.map((item: any) => ({
                        id: item.id,
                        itemNo: item.itemNo,
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-1 ml-16">{prNumber}</p>
                    </div>
                </div>
